...
### meridian strategy list
Lists all strategies in the library with a summary of each. Shows which one is currently active.
\`\`\`
Output: { active, count, strategies: [{id, name, author, lp_strategy, best_for, active, added_at}] }
\`\`\`

### meridian strategy set <id>
Sets the active strategy for the next screening/deployment cycle.
\`\`\`
Output: { active: "evil_panda_strat", name: "Evil Panda Strat (Logical TA)" }
\`\`\`

### meridian strategy get <id>
Shows full details of a specific strategy including all criteria, range, and exit rules.
\`\`\`
Output: { id, name, author, lp_strategy, token_criteria, entry, range, exit, best_for, raw, is_active }
\`\`\`

### meridian strategy add --name <text> --lp <bid_ask|spot|curve> [--author <text>] [--best-for <text>]
Adds a new strategy to the library with minimal fields. Use Telegram or the LLM for full strategy parsing from tweets.
\`\`\`
Output: { saved: true, id, name, active }
\`\`\`

### meridian strategy remove <id>
Removes a strategy from the library. If the removed strategy was active, the first remaining strategy becomes active.
\`\`\`
Output: { removed: true, id, name, new_active }
\`\`\`
...
switch (subcommand) {
  ...
  // ── strategy ─────────────────────────────────────────────────────
  case "strategy": {
    const stratLib = await import("./strategy-library.js");
    const sub3 = argv.filter(a => !a.startsWith("-"))[2];
    const action = sub2 || "list";

    switch (action) {
      case "list": {
        out(stratLib.listStrategies());
        break;
      }
      case "set": {
        if (!sub3) die("Usage: meridian strategy set <id> | tip: run 'meridian strategy list' to see IDs");
        out(stratLib.setActiveStrategy({ id: sub3 }));
        break;
      }
      case "get": {
        if (!sub3) die("Usage: meridian strategy get <id>");
        out(stratLib.getStrategy({ id: sub3 }));
        break;
      }
      case "add": {
        const name = flags.name;
        const lp = flags.lp || "bid_ask";
        const author = flags.author || "cli";
        const bestFor = flags["best-for"] || "";
        if (!name) die("Usage: meridian strategy add --name <text> --lp <bid_ask|spot|curve> [--author <text>] [--best-for <text>]");
        const slug = name.toLowerCase().replace(/\s+/g, "_").replace(/[^a-z0-9_]/g, "");
        out(stratLib.addStrategy({
          id: slug,
          name,
          author,
          lp_strategy: lp,
          best_for: bestFor,
          raw: `Added via CLI by ${author}`,
        }));
        break;
      }
      case "remove": {
        if (!sub3) die("Usage: meridian strategy remove <id>");
        out(stratLib.removeStrategy({ id: sub3 }));
        break;
      }
      default:
        die(`Unknown strategy action: ${action}. Use: list, set, get, add, remove`);
    }
    break;
  }
